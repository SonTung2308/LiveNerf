...
try:
    ...
    KAN_AVAILABLE = False
    print('[WARN] efficient_kan not installed. KAN mode will not be available.')
...
class KANMLP(nn.Module):
    """Drop-in replacement for MLP using Kolmogorov-Arnold Networks.
    
    Instead of fixed activation functions on nodes (y = σ(Wx + b)),
    KAN uses learnable B-spline activation functions on edges (y = Σ φᵢ(xᵢ)).
    This provides better function approximation with smaller networks.
    """
    def __init__(self, dim_in, dim_out, dim_hidden, num_layers, grid_size=5, spline_order=3):
        super().__init__()
        assert KAN_AVAILABLE, "efficient_kan is not installed. Run: pip install git+https://github.com/Blealtan/efficient-kan.git"
        self.dim_in = dim_in
        self.dim_out = dim_out
        self.dim_hidden = dim_hidden
        self.num_layers = num_layers

        # Build layer dimensions: [dim_in, dim_hidden, ..., dim_hidden, dim_out]
        layers_hidden = [dim_in] + [dim_hidden] * (num_layers - 1) + [dim_out]
...
class NeRFNetwork(NeRFRenderer):
    def __init__(self,
        ...
        # Select network type: KAN or MLP
        self.use_kan = getattr(opt, 'use_kan', False)
        if self.use_kan:
            assert KAN_AVAILABLE, "efficient_kan is required for --use_kan. Run: pip install git+https://github.com/Blealtan/efficient-kan.git"
            print('[INFO] Using KAN (Kolmogorov-Arnold Networks) instead of MLP')
            NetClass = KANMLP
        else:
            NetClass = MLP
        ...
        self.eye_att_net = NetClass(self.in_dim, 1, 16, 2)
        self.eye_dim = 1 if self.exp_eye else 0
        self.sigma_net = NetClass(self.in_dim + self.audio_dim + self.eye_dim, 1 + self.geo_feat_dim, self.hidden_dim, self.num_layers)
        ## color network
        self.num_layers_color = 2
        self.hidden_dim_color = 64
        self.encoder_dir, self.in_dim_dir = get_encoder('spherical_harmonics')
        self.color_net = NetClass(self.in_dim_dir + self.geo_feat_dim + self.individual_dim, 3, self.hidden_dim_color, self.num_layers_color)

        self.unc_net = NetClass(self.in_dim, 1, 32, 2)

        self.aud_ch_att_net = NetClass(self.in_dim, self.audio_dim, 64, 2)
        ...
        if self.torso:
            ...
            self.torso_deform_net = NetClass(self.torso_deform_in_dim + self.anchor_in_dim + self.individual_dim_torso, 2, 32, 3)
            ...
            self.torso_net = NetClass(self.torso_in_dim + self.torso_deform_in_dim + self.anchor_in_dim + self.individual_dim_torso, 4, 32, 3)
